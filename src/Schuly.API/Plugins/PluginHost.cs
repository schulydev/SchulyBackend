...
namespace Schuly.API.Plugins
{
    ...
    public sealed class PluginHost(IServiceProvider rootProvider, IConfiguration configuration, ApplicationPartManager partManager, PluginEndpointDataSource endpointSource, PluginAssemblyMap assemblyMap, PluginSchedulerRegistry scheduler, ILogger<PluginHost> logger) : IPluginEventDispatcher
    {
        ...
        public async Task LoadAsync(PluginManifest manifest, string pluginDirectory, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                try
                {
                    provider = BuildChildProvider(plugin);
